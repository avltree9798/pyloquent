...
class Model(BaseModel, metaclass=ModelMeta):
    ...
    async def _perform_insert(self: T) -> T:
        ...
        if is_composite:
            ...
            id = None
        elif not self.__class__.__incrementing__:
            # Non-incrementing primary keys (e.g. UUID / string keys) are
            # supplied by the caller, not generated by the database. Insert the
            # row as-is and keep the caller's key — never overwrite it with the
            # driver's integer lastrowid.
            await query.insert(attributes)
            id = self._get_key()
        else:
